...
function search() {
    $("#search input").on('keyup', function () {
        ...
        const JUNGSUNG = ['ㅏ', 'ㅐ', 'ㅑ', 'ㅒ', 'ㅓ', 'ㅔ', 'ㅕ', 'ㅖ', 'ㅗ', 'ㅗㅏ', 'ㅗㅐ', 'ㅗㅣ', 'ㅛ', 'ㅜ', 'ㅜㅓ', 'ㅜㅔ', 'ㅜㅣ', 'ㅠ', 'ㅡ', 'ㅡㅣ', 'ㅣ']
                                        ...
                                        .toLocaleUpperCase())
        {
            if ('가' <= w && w <='힣')
            {
                ...
                r_lst = r_lst.concat(CHOSUNG[ch1] + JUNGSUNG[ch2] + JONGSUNG[ch3])
            }
            else
                r_lst.push(w)
        }
...
function showStat(code)
{
    function translate(s)
    {
        return s.replace('AP', '공격력')
                ...
                .replace('CC', '치명타 확률')
                .replace('CD', '치명타 피해량')
                .replace('SA', '스킬 증폭').replace('SA', '스킬 증폭')
                .replace('CR', '쿨타임 감소')
                .replace('MS', '최대 스태미너')
